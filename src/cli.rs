...
//! - tui: Launch the interactive terminal UI (also the default if no command is
//!   given)

use anyhow::Result;
use clap::{Parser, Subcommand};

use crate::{db::{Database, Note}, utils::{format_date_full, format_date_only, note_to_markdown, parse_markdown_file, parse_tags, resolve_note, sanitize_filename}};
...
pub fn handle_command(db: &Database, cmd: Commands) -> Result<()> {
	match cmd {
		Commands::Add { title, content, tags } => cmd_add(db, title, content, tags),
		Commands::List { tag, oneline, sort, limit } => cmd_list(db, tag, oneline, sort, limit),
		Commands::Show { id_or_title } => cmd_show(db, &id_or_title),
		Commands::Edit { id_or_title, title, content, tags } => cmd_edit(db, &id_or_title, title, content, tags),
		Commands::Delete { id_or_title, yes } => cmd_delete(db, &id_or_title, yes),
		Commands::Search { query } => cmd_search(db, &query),
		Commands::Export { id_or_title, output } => cmd_export(db, &id_or_title, output),
